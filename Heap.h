...
// Size of array will be nVerts (number of vertices in graph)
struct Graph
{
	int nVerts;
...
// A utility function that creates a graph of nVerts vertices
struct Graph* createGraph(int nVerts)
{
	struct Graph* graph = (struct Graph*) malloc(sizeof(struct Graph));
	graph->nVerts = nVerts;
 
	// Create an array of adjacency lists.  Size of array will be V
	graph->array = (struct AdjList*) malloc(nVerts * sizeof(struct AdjList));
...
void printArr(float dist[], int n, ofstream* filePtr)
{
	for (int i = 0; i < n; ++i)
		*filePtr << dist[i] << " ";
	*filePtr << endl;
...
void calculateGeodesicByHeap(struct Graph* graph, const char* outputFile)
{ 
	int nVerts = graph->nVerts;	// Get the number of vertices in graph
	...
	for (int src=0; src < nVerts; src++) {
		...
		float dist[nVerts];		// dist values used to pick minimum weight edge in cut

		// minHeap represents set E
		struct MinHeap* minHeap = createMinHeap(nVerts);
		...
		printArr(dist, nVerts, &myFile);
	}

	myFile.close();
	timeMeasure = clock() - timeMeasure;
	cout << "It took " << ((float)timeMeasure)/CLOCKS_PER_SEC << "seconds to calculate all geodesics by HEAP.\n";
}
